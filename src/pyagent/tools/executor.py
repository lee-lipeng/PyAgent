...
class ToolExecutor:
    ...
    Args:
        registry: 工具注册表。
        hooks: 事件总线（HookManager），执行前后 emit 事件。
    ...
    async def _exec_one(
        ...
        流程：
        1. emit BEFORE_TOOL（Permission Hook 可在此抛异常拦截）
        ...
        try:
            ...
            await self._emit_after(tool_call, result, is_error=True)
            return result
        ...
        try:
            result = await tool.execute(tool_call.id, validated_args, signal=signal)
        except PermissionError:
            # Permission Hook 抛出的异常，直接传播
            raise
        except Exception as exc:
        ...
        await self._emit_after(tool_call, result, result.is_error)
        return result

    async def _emit_after(self, tool_call: ToolCall, result: ToolResult, is_error: bool) -> None:
        """emit AFTER_TOOL 事件。"""
        await self._hooks.emit(
            self._make_event(
                "after_tool",
